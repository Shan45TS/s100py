...
def create_s111(output_file, dcf) -> S111File:
    ...
    surface_current_feature_dataset = (
        root.feature_information.surface_current_feature_dataset
    )
...
def add_metadata(metadata: dict, data_file) -> S111File:
    ...
    surface_current_feature_instance_01 = (
        surface_current_feature.surface_current.append_new_item()
    )
    ...
    speed_uncertainty = (
        surface_current_feature_instance_01.uncertainty_dataset.append_new_item()
    )
    ...
    surface_current_feature.horizontal_position_uncertainty = metadata[
    ...
    surface_current_feature_instance_01.date_time_of_first_record = metadata[
    ...
    if "depthTypeIndex" in metadata:
        if metadata["depthTypeIndex"] == 1:
            ...
            root.vertical_datum_reference = metadata.get(
                "verticalDatumReference", VERTICAL_DATUM_REFERENCE.s100VerticalDatum
            )
            if metadata["verticalDatumReference"] == 1:
                root.vertical_datum = metadata.get(
                    "verticalDatum", VERTICAL_DATUM.seaSurface
                )
...
def add_data_from_arrays(
    ...
    if data_coding_format == 2:
        ...
        surface_current_feature_instance_01.grid_origin_longitude = grid_properties[
            "minx"
        ]
        surface_current_feature_instance_01.grid_origin_latitude = grid_properties[
            "miny"
        ]
        surface_current_feature_instance_01.grid_spacing_longitudinal = grid_properties[
            "cellsize_x"
        ]
        surface_current_feature_instance_01.grid_spacing_latitudinal = grid_properties[
            "cellsize_y"
        ]

        surface_current_feature_instance_01.num_points_latitudinal = grid_properties[
            "ny"
        ]
        surface_current_feature_instance_01.num_points_longitudinal = grid_properties[
            "nx"
        ]
    ...
    surface_current_group_object = (
        surface_current_feature_instance_01.surface_current_group.append_new_item()
    )
...
def update_metadata(data_file, grid_properties: dict, update_meta: dict) -> S111File:
    ...
    surface_current_feature_instance_01.date_time_of_last_record = update_meta[
    ...
    surface_current_feature_instance_01.time_record_interval = update_meta[
        "timeRecordInterval"
    ]
