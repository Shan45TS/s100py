...
from ...s100.v5_0.api import (
    S100File,
    S100Root,
    S100Exception,
    FeatureContainerDCF2,
    FeatureInstanceDCF2,
    FeatureContainerDCF3,
    FeatureInstanceDCF3,
    FeatureContainerDCF7,
    FeatureInstanceDCF7,
    FeatureInformation,
    FeatureInformationDataset,
    GroupFBase,
    VERTICAL_CS,
...
class WaterLevelValues(S1xxGridsBase):
    ...
    def water_level_height_create(self):
        ...
        # noinspection PyAttributeOutsideInit
        # pylint: disable=attribute-defined-outside-init
        self.water_level_height = self.__water_level_height_type__(
            [], self.water_level_height_dtype
        )
    ...
    def water_level_trend(self, val: Union[int, str, WaterLevelTrend]):
        self.set_enum_attribute(
            val, self.__water_level_trend_hdf_name__, self.__water_level_trend_type__
        )
    ...
    def water_level_trend_dtype(self) -> Type[WaterLevelTrend]:
        ...
        return h5py.enum_dtype(
            dict(
                [
                    (water_level_trend.name, water_level_trend.value)
    ...
    def water_level_trend_create(self):
        ...
        # noinspection PyAttributeOutsideInit
        # pylint: disable=attribute-defined-outside-init
        self.water_level_trend = self.__water_level_trend_type__(
            [], self.water_level_trend_dtype
        )

    def get_write_order(self):
        return [
            self.__water_level_height_hdf_name__,
            self.__water_level_trend_hdf_name__,
        ]
...
class WaterLevelGroup(S1xxObject):
    ...
    def time_point(self, val: Union[datetime.date, datetime.datetime, str]):
        self.set_datetime_attribute(
            val, self.__time_point_hdf_name__, self.__time_point_type__
        )
...
class WaterLevelFeatureInstanceBase:
    ...
    def data_dynamicity(self, val: Union[int, str, DATA_DYNAMICITY]):
        self.set_enum_attribute(
            val, self.__data_dynamicity_hdf_name__, self.__data_dynamicity_type__
        )
...
class GroupF(GroupFBase):
    ...
    def feature_code_create(self):
        # noinspection PyAttributeOutsideInit
        # pylint: disable=attribute-defined-outside-init
        self.feature_code = self.__feature_code_type__(
            [WATER_LEVEL], dtype=h5py_string_dtype
        )
...
class S104Root(S100Root):
    ...
    def water_level_create(self):
        ...
        # pylint: disable=attribute-defined-outside-init
        print(
            "You should not create a generic surface_current object but manually create"
            "a WaterLevelContainerDCF2 or WaterLevelContainerDCF3"
        )
        raise S104UnspecifiedClassException(
            "You should create WaterLevelContainerDCFx (x=2,3,8 etc)"
        )
        self.water_level = self.__water_level_type__()
...
class S104File(S100File):
    ...
    def __init__(self, *args, **kywrds):
        ...
        try:
            ...
            KeyError
        ):  # must not be reading an existing file or doesn't have data for some reason
            pass
        else:
